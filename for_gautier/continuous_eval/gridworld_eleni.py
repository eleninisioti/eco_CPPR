    ...
    def __init__(self,
        ...
        def reset_fn(key):
            ...
            if self.place_resources:
                ...
                pos_food_x = jnp.concatenate(
                    (random.randint(next_key, (int(init_food / 4),), int(1 / 2 * SX) + N, (SX - 1)),
                     random.randint(next_key, (int(init_food / 4),), 1, int(1 / 2 * SX) - N),
                ...
                pos_food_y = jnp.concatenate((random.randint(next_key, (int(init_food / 4),), 1, SY - 1),
                                              ...
                                              random.randint(next_key, (int(init_food / 4),),  int(1 / 2 * SY) + N,
                                                             (SY - 1)),
                                              random.randint(next_key, (int(init_food / 4),), 1, int(1 / 2 * SY) - N)))
                next_key, key = random.split(key)
        ...
        def step_fn(state, actions):
            ...
            if scale_constant != 0:
                ...
                num_neighbs = jnp.where(num_neighbs == 1, 0.01/scale_constant, num_neighbs)
